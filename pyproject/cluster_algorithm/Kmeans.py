...
user = pd.read_csv("dataset/Users_300")  # 玻璃辨识数据集 Glass Identification  class=6
user_2 = pd.read_csv("dataset/Users_300_2")  # 玻璃辨识数据集 Glass Identification  class=6
user_2 = user_2[0 :100]
df = user_2  # 设置要读取的数据集
# print(df)
...
dataset = df  # 预处理之后的数据，去除掉了第一行的数据（因为其为特征名，如果数据第一行不是特征名，可跳过这一步）
...
plt.title('K_Means + DR '+'cluster:'+str(k))
plt.show()
